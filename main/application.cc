...
#error "CONFIG_USE_DEVICE_AEC and CONFIG_USE_SERVER_AEC cannot be enabled at the same time"
    ...
    // 创建重连定时器（caption 设备断线自动重连）
    esp_timer_create_args_t reconnect_timer_args = {
        .callback = [](void* arg) {
            Application* app = (Application*)arg;
            xEventGroupSetBits(app->event_group_, MAIN_EVENT_SCHEDULE);
            app->Schedule([app]() { app->HandleReconnectEvent(); });
        },
        .arg = this,
        .dispatch_method = ESP_TIMER_TASK,
...
void Application::InitializeProtocol() {
    ...
    protocol_->OnAudioChannelClosed([this, &board]() {
        ...
        Schedule([this]() {
            auto display = Board::GetInstance().GetDisplay();
            if (caption_mode_) {
                // caption 设备：不清空字幕，显示"重连中"，启动自动重连
    ...
    protocol_->OnIncomingJson([this, display](const cJSON* root) {
        ...
        if (strcmp(type->valuestring, "tts") == 0) {
            ...
            if (cJSON_IsString(text)) {
                ESP_LOGI(TAG, ">> %s", text->valuestring);
                Schedule([display, message = std::string(text->valuestring), this]() {
                    if (caption_mode_) {
...
void Application::HandleStateChangedEvent() {
    ...
    switch (new_state) {
        ...
        case kDeviceStateIdle:
            ...
            // caption 设备不需要唤醒词检测（听障用户可能无法说话）
            audio_service_.EnableWakeWordDetection(!caption_mode_);
            break;
...
void Application::StartReconnectTimer() {
    ...
    int delays[] = {1, 2, 4, 8, 16, 30, 30, 30, 30, 30};
    int delay = delays[std::min(reconnect_retry_count_, 9)];
    ESP_LOGI(TAG, "启动重连定时器，%d 秒后重连（第 %d 次）", delay, reconnect_retry_count_ + 1);
    esp_timer_stop(reconnect_timer_handle_);
    esp_timer_start_once(reconnect_timer_handle_, delay * 1000000ULL);
}
...
void Application::HandleReconnectEvent() {
    if (!caption_mode_) {
        return;
    }
    // 如果已经在监听状态，说明已经连接成功
    ...
    if (reconnect_retry_count_ > MAX_RECONNECT_RETRIES) {
        ESP_LOGE(TAG, "重连次数超过上限 %d，停止重连", MAX_RECONNECT_RETRIES);
        auto display = Board::GetInstance().GetDisplay();
        display->SetStatus("连接失败，请检查网络");
        return;
    }
    ESP_LOGI(TAG, "正在自动重连（第 %d 次）...", reconnect_retry_count_);
    SetDeviceState(kDeviceStateConnecting);
