...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    if (strcmp(role, "caption") == 0) {
        ...
        // 避免每次 ASR 中间结果都 lv_obj_clean 重建（每秒 5-10 次）
        if (chat_message_label_ != nullptr &&
...
void LcdDisplay::SetChatMessage(const char* role, const char* content) {
    ...
    // caption 角色特殊处理：占据大部分屏幕，居中换行显示
    if (strcmp(role, "caption") == 0) {
        if (bottom_bar_ != nullptr) {
            lv_obj_remove_flag(bottom_bar_, LV_OBJ_FLAG_HIDDEN);
            // 扩展 bottom_bar 到屏幕 70% 高度，居中显示
            lv_obj_set_height(bottom_bar_, LV_VER_RES * 70 / 100);
            lv_obj_align(bottom_bar_, LV_ALIGN_CENTER, 0, 0);
            lv_obj_set_style_pad_all(bottom_bar_, lvgl_theme->spacing(8), 0);
        }
        lv_label_set_long_mode(chat_message_label_, LV_LABEL_LONG_WRAP);
        ...
        // 非 caption 角色：恢复原始 bottom_bar 布局
        if (bottom_bar_ != nullptr) {
#if CONFIG_USE_MULTILINE_CHAT_MESSAGE
            lv_obj_set_height(bottom_bar_, LV_SIZE_CONTENT);
#else
            lv_obj_set_height(bottom_bar_, text_font->line_height + lvgl_theme->spacing(8));
#endif
            lv_obj_align(bottom_bar_, LV_ALIGN_BOTTOM_MID, 0, 0);
            lv_obj_set_style_pad_all(bottom_bar_, lvgl_theme->spacing(4), 0);
        }
    ...
    if (bottom_bar_ != nullptr && strcmp(role, "caption") != 0) {
        lv_obj_align(bottom_bar_, LV_ALIGN_BOTTOM_MID, 0, 0);
    }
#endif
}
